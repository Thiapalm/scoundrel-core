...
GamePhase DefaultGameLogic::initialize_game(GameContext &ctx)
{
    if (!ctx.get_player() || !ctx.get_player()->player_class || ctx.get_player()->current_life() == 0) {
        auto player = create_player(PlayerClassType::PEASANT);
...
GamePhase DefaultGameLogic::pre_turn(GameContext &ctx)
{
    return GamePhase::START_TURN;
}

GamePhase DefaultGameLogic::start_turn(GameContext &ctx)
{
...
GamePhase DefaultGameLogic::pre_combat(GameContext &ctx)
{
    ctx.get_player()->trigger_passives(ctx, PassiveTrigger::BEFORE_COMBAT);
    return GamePhase::COMBAT;
}

GamePhase DefaultGameLogic::combat(GameContext &ctx)
{
    while (true) {
        try {
...
GamePhase DefaultGameLogic::post_combat(GameContext &ctx)
{
    ctx.get_player()->trigger_passives(ctx, PassiveTrigger::AFTER_COMBAT);
    return GamePhase::TERMINATE_TURN;
}

GamePhase DefaultGameLogic::terminate_turn(GameContext &ctx)
{
...
GamePhase DefaultGameLogic::post_turn(GameContext &ctx)
{
    return GamePhase::START_TURN;
}


int DefaultGameLogic::calculate_loosing_score(GameContext &ctx)
{
    int result{0};
...
GamePhase DefaultGameLogic::end_game(GameContext &ctx)
{
    if (ctx.is_victory())
    {
